                        ...
                        @foreach ($offers as $offer)
                            ...
                            <tr>
                                @if(LaravelLocalization::getCurrentLocale() == 'en') 
                                    ...
                                    <td><img class="rounded-circle" src="{{ asset('images/offers/' . $offer->photo) }}" alt="" width="50" height="50"></td>
                                ...
                                <td class="text-center">
                                    ...
                                    <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                        <button type="button" class="btn btn-success dropdown-item" data-toggle="modal" data-target="#modal-edit{{$offer->id}}" ><i  class="fas fa-user-edit"></i>التحرير</button>
                                        <button type="button" class="dropdown-item" data-toggle="modal" data-target="#modal-delete{{$offer->id}}" ><i style="color:red" class="fas fa-user-minus"></i>الحذف</button>
                                    </div>
                                </td>
                                        ...
                                        <form  method="POST" action="{{route('offers.update',$offer->id)}}"  enctype="multipart/form-data">
                                                ...
                                                @if(LaravelLocalization::getCurrentLocale() == 'en')
                                                    <div class="form-group">
                                                        ...
                                                        <input name="name_en" type="text" class="form-control" id="exampleInputEmail1" placeholder="Enter name" required value="{{$offer->name}}" required>
                                                    </div>
                                                    <div class="form-group">
                                                        <label for="exampleInputEmail1">الوصف بالإنكلزي</label>
                                                        <textarea class="form-control mg-t-20" name="description_en" value="" placeholder="أدخل الوصف" required autocomplete="false" >{{$offer->description}}</textarea>
                                                    </div>
                                                @elseif(LaravelLocalization::getCurrentLocale() == 'ar')
                                                    <div class="form-group">
                                                        <label for="exampleInputEmail1">الأسم بالعربي</label>
                                                        <input name="name_ar" type="text" class="form-control" id="exampleInputEmail1" placeholder="Enter email" required value="{{$offer->name}}" required>
                                                    </div>
                                                    
                                                    <div class="form-group">
                                                        <label for="exampleInputEmail1">الوصف بالعربي</label>
                                                        <textarea class="form-control mg-t-20" name="description_ar"  placeholder="أدخل الوصف" required autocomplete="false" >{{$offer->description}}</textarea>
                                                    </div>
                                                    @endif
                                                    <div class="form-group">
                                                        <label for="exampleInputEmail1">من التاريخ</label>
                                                        <input name="start_date" value="{{ $offer->start_date }}" type="date" class="form-control" required autocomplete="off">
                                                    </div>
                                                    <div class="form-group">
                                                        <label for="exampleInputEmail1">إلى التاريخ</label>
                                                        <input name="end_date" type="date" value="{{ $offer->end_date }}" class="form-control" required autocomplete="off">
                                                    </div>
                                                    <div class="form-group">
                                                        <label for="exampleInputEmail1">حالة العرض</label>
                                                        <select name="status" class="form-control select-lg select2">
